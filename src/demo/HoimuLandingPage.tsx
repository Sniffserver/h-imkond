...
import { DemoLandingPageProps, DemoBatteryStatus, DemoWikiArticle } from './types';
import { usePWAInstall } from '../hooks/usePWAInstall';

export const HoimuLandingPage: React.FC<DemoLandingPageProps> = ({
  onEnterApp,
  onNavigateToTab,
  isNightMode = false,
  onToggleNightMode,
...
volumes:
  ...
  const wikiArticles: DemoWikiArticle[] = [
    {
      id: 'dtn-ble',
      category: 'mesh',
  ...
  return (
    ...
    >
      {/* =========================================================================
          CRITICAL DISCLAIMER: DEMO / SIMULATION ENVIRONMENT ONLY
          Isolated marketing & sandbox demo — Never shares production services/models
      ========================================================================= */}
      <div className="bg-amber-500 text-stone-950 px-4 py-2 text-xs font-mono font-bold flex flex-wrap items-center justify-between gap-2 shadow-sm border-b border-amber-600 z-50">
        <div className="flex items-center gap-2">
          <span className="px-2 py-0.5 rounded bg-black text-amber-300 font-black text-[10px] tracking-wider uppercase">
            DEMO / SIMULATION
          </span>
          <span>Interactive Demonstration Sandbox — Simulated packet logs, synthetic progress & demo infrastructure. Not live mesh.</span>
        </div>
        <button
          type="button"
          onClick={onEnterApp}
          className="px-3 py-1 rounded-lg bg-black text-white hover:bg-stone-800 text-[11px] font-bold cursor-pointer transition-colors shadow-xs"
        >
          Launch Live Product →
        </button>
      </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#E9C46A] font-bold">[SIMULATOR] BIOREGION MESH SANDBOX</span>
            <span className="text-white/40 hidden sm:inline">•</span>
            <span className="hidden sm:inline text-[#A8BDA5]">
              {peersCount} Simulated Nodes in Range · Zero-Cloud P2P
            </span>
          </div>
